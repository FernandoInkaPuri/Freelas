...
describe 'Project API' do 
  ...
  context 'POST /api/v1/projects' do
    it 'should save a new project' do
      ...
      expect(parsed_body[:limit_date]).to eq(Date.parse(I18n.localize(1.week.from_now.to_date)).to_s)
      expect(parsed_body[:start_date]).to eq(Date.parse(I18n.localize(2.weeks.from_now.to_date)).to_s)
      expect(parsed_body[:end_date]).to eq(Date.parse(I18n.localize(2.months.from_now.to_date)).to_s)
